...
def prompt_to_event(user_prompt: str) -> Dict:
    """
    Ask Gemini to convert a argument - short Polish instruction into one JSON:
    ...
    gemini_instructions = (
        "You convert Polish natural language into exactly ONE Google Calendar event.\n"
        f"Today is {today.date().isoformat()} and the current local time is {today.time().strftime('%H:%M')} in Europe/Warsaw.\n"
        "You MUST return exactly one valid JSON object, NOT an array, NOT multiple objects.\n"
        "The JSON must have EXACTLY these keys: title, start_iso, end_iso, timezone.\n"
        ...
        "Do NOT include any explanation, text, or comments outside the JSON."
    )
    ...
    while True:
        ...
        print(response.text)  # Print the model's response for debugging
...
def list_events(calendar_id="primary", max_results=10, time_min=None, time_max=None):
    """List events from Google Calendar within a specific time range. """
    calendar_service = get_calendar_service()

    if not time_min:
        time_min = dt.datetime.now(TZ).isoformat()
    
    events_results = calendar_service.events().list(
    ...
    if not events:
        ...
        return
    for event in events:
        print(f"🗓️ {event['summary']} (Start: {event['start']['dateTime']}, End: {event['end']['dateTime']})")
